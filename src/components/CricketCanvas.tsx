...
GitHub
 repo:sanjivanishow/cricket-grammar-game import

Imported sanjivanishow/cricket-grammar-game from GitHub.

Default branch: main

Visibility: Public

Access confirmed: Admin and push permissions available

Repository is active and ready for inspection or modification.
...
svg-sprite.svg
File
the thing I want the game to have graphics like this google doodle, but it is not working

Worked for 9m 13s
I found the problem
The current renderer changes the SVG sprite sheet from its native 596.4 × 9,965 coordinate system to 10,000 × 10,000, but continues cropping it with the original coordinates. Canvas therefore reads the wrong portions of the sprite sheet, causing blank, distorted, or unrelated graphics.

Several sprite mappings were also incorrect:

7058 and 7262 are wicket-breaking animation frames, not crowd characters.

810 is a wheat stalk, not a tree.

The batter graphic did not include the separate bat sprite.

The actual crowd and stadium sprites were not being used.

I checked these against the uploaded original Doodle HTML and JavaScript. 
...
src/components/CricketCanvas.tsx
with:

CricketCanvas.fixed.tsx

The replacement now includes:

Correct native SVG loading without resizing the sprite sheet

Original 5-pixel sprite cropping buffer

Correct stadium and animated crowd

Cricket batter with a separate moving bat

Snail bowler

Doodle wheat, clouds and field graphics

Original animated wicket-breaking sequence

Original dirt-impact animation

Ball flight for six, four, three, two, one, dot ball and run-out

Boundary confetti, trophy and original OUT! graphic

Correct 5:3 aspect ratio to prevent stretching

Existing CricketOutcome and onAnimationComplete compatibility
...
DOODLE_GRAPHICS_FIX.md

I attempted both a new branch and a direct update to main, but the connected GitHub integration returned 403: Resource not accessible by integration for write operations. The repository therefore remains unchanged until the replacement file is uploaded manually or the GitHub app is granted repository-content write permission.

the graphics still does not look smooth t
